...
let expConvolution = db.Experiments.Where(fun (x:Experiments) -> x.Id = 4).First()
let expReduce = db.Experiments.Where(fun (x:Experiments) -> x.Id = 11).First()
let expSaxpy = db.Experiments.Where(fun (x:Experiments) -> x.Id = 7).First()

let exps = [| expConvolution ; expReduce ; expSaxpy |]

let expCasesConvolution = db.ExperimentCases.Where(fun (x:ExperimentCases) -> x.Experiment_id = expConvolution.Id )
let expCasesReduce = db.ExperimentCases.Where(fun (x:ExperimentCases) -> x.Experiment_id = expReduce.Id )
let expCasesSaxpy = db.ExperimentCases.Where(fun (x:ExperimentCases) -> x.Experiment_id = expSaxpy.Id )
...
let handleCase (case:ExperimentCases) (exp:Experiments)=
    ...
    let nameArray =  match args.[0] with
                    | "saxpy_openCL" -> [| 0. |].ToArray()
                    | "convolution" -> [| 1. |].ToArray()
                    | "reduce" -> [| 2. |].ToArray()
                    | _ -> [| 1. |].ToArray()
    ...
    Array.concat [| nameArray; argArray; valArray; [| j ; mat_size |] |]

let data cases (e:Experiments) =
    cases |> Seq.map (fun (c:ExperimentCases) -> (handleCase c e))
...
let colNames (e:Experiments) (c:ExperimentCases) (r:ExperimentRuns)=
    ...
    Array.concat [| name ; args ; sensorsNames ; [|"J"; "matrix_size"|] |]

let names = colNames (exp) (expCases.First()) (Seq.head (expRuns (expCases.First())))
...
//for round in 0..2 do    
    ...
    let valuesMatrix = data casesSubset exp
